...
function Scatter (options) {
	...
	function update (options) {
		...
		options = pick(options, {
			...
			borderSize: 'borderSizes borderSize stroke-width',
			color: 'colors color fill fill-color',
		...
		if (options.precise != null) precise = options.precise
		...
		if (options.positions && options.positions.length) {
			...
			if (options.positions[0].length) {
				...
				unrolled = precise ? new Float64Array(options.positions.length) : new Float32Array(options.positions.length)
			...
			//hi-precision buffer has normalized coords and [hi,hi, lo,lo, hi,hi, lo,lo...] layout
				...
				let boundX = (bounds[2] - bounds[0]) || 1., boundY = (bounds[3] - bounds[1]) || 1.
		...
		if (options.range || options.positions) {
			...
			pixelSize = (range[2] - range[0]) / gl.drawingBufferWidth
		}
